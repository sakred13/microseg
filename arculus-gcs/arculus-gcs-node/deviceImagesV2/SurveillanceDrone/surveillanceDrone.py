...
def start_mission():
    ...
    data = decrypt_json(request.data)
    ...
    return Response(encrypt_json({"message": "Mission started", "X_COORD": X_COORD, "Y_COORD": Y_COORD, "ENEMY_X": ENEMY_X, "ENEMY_Y": ENEMY_Y, "ENEMY_RADIUS": ENEMY_RADIUS}, encryptionKey), mimetype='text/plain')

@app.route('/commandToMove', methods=['POST'])
def command_to_move():
    global X_COORD, Y_COORD, encryptionKey
    ...
    if is_within_enemy_radius(new_x, new_y):
        ...
        return Response(encrypt_json({"message": "Found air defense", "X_COORD": X_COORD, "Y_COORD": Y_COORD, "TURN_POINT_X": TURN_POINT_X, "TURN_POINT_Y": TURN_POINT_Y, "ENEMY_RADIUS": ENEMY_RADIUS, "ENEMY_X": ENEMY_X, "ENEMY_Y": ENEMY_Y}, encryptionKey), mimetype='text/plain')

    if is_within_dest_radius(new_x, new_y, move_distance):
        return Response(encrypt_json({"message": "Reached Destination", "X_COORD": X_COORD, "Y_COORD": Y_COORD}, encryptionKey), mimetype='text/plain')
    
    # Update coordinates
    X_COORD = new_x
    Y_COORD = new_y
    return Response(encrypt_json({"message": "Moved", "X_COORD": X_COORD, "Y_COORD": Y_COORD, "slope": slope}, encryptionKey), mimetype='text/plain')
